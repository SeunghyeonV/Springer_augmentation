...
transforms_list = [transforms.Compose([CIFAR10Policy_autocontrast(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_brightness(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_color(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_contrast(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_equalize(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_invert(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_posterize(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_rotate(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_sharpness(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_shearX(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_shearY(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_solarize(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_translateX(magnitude=4), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_translateY(magnitude=4), transforms.ToTensor()]),
                   ]
...
#################################################################################################
def input_savedir_M4():
    ## get filename
    default_address = "/home/iis/Desktop/CIFAR100_reconstruction/cifar100_output/randaug_M4/model_accuracy/"
    os.makedirs(default_address) if not os.path.exists(default_address) else None
    input_address_first = []

    # generates C:\Users\seacl\Desktop\CIFAR100_reconstruction\cifar100_output\randaug_M4\model_accuracy\ 14 aug...
...
def output_savedir_M4():
    ## get filename
    default_address = "/home/iis/Desktop/CIFAR100_reconstruction/cifar100_output/randaug_M4/attack_accuracy/"
    os.makedirs(default_address) if not os.path.exists(default_address) else None
...
def psnr_savedir_M4():
    ...
    default_address = "/home/iis/Desktop/CIFAR100_reconstruction/cifar100_output/randaug_M4/psnr"
    os.makedirs(default_address) if not os.path.exists(default_address) else None
...
input_savepath = input_savedir_M4()
output_savepath = output_savedir_M4()
psnr_savepath = psnr_savedir_M4()
num_files = 10

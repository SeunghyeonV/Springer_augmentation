...
transforms_list = [transforms.Compose([CIFAR10Policy_autocontrast(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_brightness(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_color(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_contrast(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_equalize(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_invert(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_posterize(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_rotate(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_sharpness(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_shearX(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_shearY(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_solarize(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_translateX(magnitude=1), transforms.ToTensor()]),
                   transforms.Compose([CIFAR10Policy_translateY(magnitude=1), transforms.ToTensor()]),
                   ]
...
def input_savedir_M1():
    ## get filename
    default_address = "/home/iis/Desktop/CIFAR100_reconstruction/cifar100_output/randaug_M1/model_accuracy/"
    os.makedirs(default_address) if not os.path.exists(default_address) else None
    input_address_first = []

    # generates C:\Users\seacl\Desktop\CIFAR100_reconstruction\cifar100_output\randaug_M1\model_accuracy\ 14 aug...
...
def output_savedir_M1():
    ## get filename
    default_address = "/home/iis/Desktop/CIFAR100_reconstruction/cifar100_output/randaug_M1/attack_accuracy/"
    os.makedirs(default_address) if not os.path.exists(default_address) else None
...
def psnr_savedir_M1():
    ...
    default_address = "/home/iis/Desktop/CIFAR100_reconstruction/cifar100_output/randaug_M1/psnr"
    os.makedirs(default_address) if not os.path.exists(default_address) else None
